...
1. From the Cloud Shell, run the following command to set a variable named location with an Azure Region near you (replace the '&lt;Azure region&gt;' placeholder with the name of the Azure region that is available for deployment of Azure VMs in your subscription, for example 'eastus'. Note: 'westus' will NOT allow you to choose VM size later, so do not use!):
